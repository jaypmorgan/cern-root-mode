...
(ert-deftest root-test-push-new ()
  ...
  (should (equal (root--push-new "a" (list "b" "c")) (list "a" "b" "c"))))

(ert-deftest root-test-pluck-item ()
  "Tests the functionality of plucking an item from a list as defined in root.el"
  (should (equal (root--pluck-item 'a '((a . b))) 'b))
  (should (equal (root--pluck-item 'a '((b . c))) nil))
  (should (equal (root--pluck-item "a" '(("a" . 3))) 3))
  (should (equal (root--pluck-item :a '((:a . b))) 'b))
  (should (equal (root--pluck-item 'a '((a . b) (a . c))) 'b)))

(ert-deftest root-test-make-earmuff ()
  "Tests that a string can be given earmuffs, i.e. name -> *name*"
  (should (equal (root--make-earmuff "name") "*name*"))
  (should (equal (root--make-earmuff "*name*") "*name*"))
  (should (equal (root--make-earmuff "*name") "**name*"))
  (should (equal (root--make-earmuff "") ""))
  (should (equal (root--make-earmuff "a") "*a*")))

(ert-deftest root-test-make-no-earmuff ()
  "Tests that earmuffs can be removed from strings"
  (should (equal (root--make-no-earmuff "*name*") "name"))
  (should (equal (root--make-no-earmuff "name") "name"))
  (should (equal (root--make-no-earmuff "") ""))
  (should (equal (root--make-no-earmuff "a") "a")))
